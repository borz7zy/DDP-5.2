/*
*
*	"Если мод не выдает при компиляции 2281337 ошибок - мод говно" (с) Drevniy.
*
*/

/*
*
*	"А МДС СОСЕТ УОУОУОУОООООО" (с) Drevniy.
*
*/

/*
*
*   если в описании системы более одного раза написано "по идее" - значит система работает на 150% (с) Drevniy.
*
*/

/*
*
*	если ваш мод выдает ошибки после первой компиляции - это нормально, но если после 20того... - не расстраивайтесь, возможно вы просто криворукий хуесос (с) Drevniy.
*
*/

/*
*
*   Author: dzh1nb0y.oldsc. VK: vk.com/dzhinboy .
*
*/
...
#define NS "DDP«"
#define MNS "«"
#define IP_S "185.13.47.58:7777"
#define GEN_D "kyvaldaaa"
#define DEVONE "dzh1nb0y.oldsc"
#define VK_G "vk.com/ddp.samp"
#define VK_GEN_D "vk.com/ughgvs"
#define VK_DEV "vk.com/dzhinboy"
#define COLOR_SERVER "{8F30E4}"
#define DIALOG_ZERO 2
...
new Float:playspax[5] = {153.6635,2180.6038,388.6408,1127.0583,-2049.1440};//координаты x случайного спавна игрока
new Float:playspay[5] = {-1940.3774,-2272.9414,-1523.2218,-1446.9573,461.4249};//координаты y случайного спавна игрока
new Float:playspaz[5] = {3.7734,13.4850,32.2734,15.4261,35.1719};//координаты z случайного спавна игрока
new Float:playspaa[5] = {359.1931, 82.77, 223.5877, 83.28,314.0035};//угол поворота случайного спавна игрока

new ColNick[19] = {0xFF0000FF, 0xFF3F3FFF, 0xFF3F00FF, 0xBF3F00FF, 0xFF7F3FFF, 0xFF7F00FF, 0xFFFF00FF, 0x3FFF3FFF, 0x00FF00FF,
0x00BF00FF, 0x00FFFFFF, 0x00BFFFFF, 0x3F3FFFFF, 0x0000FFFF, 0x7F3FFFFF, 0x7F00FFFF, 0xFF00FFFF, 0x7F7F7FFF, 0x00FF0000};//массив цветов ника
...
static const Float: checkpoint_fire [] [3] =
{
...
//DUELSYS
	...
	for(new i=0;i<sizeof(playspax);i++){
...
public OnPlayerText(playerid, text[])
{
	...
	if(text[0] == '@' || text[0] == '#' || text[0] == '№')
	{
	    ...
	    if(sscanf(pText, "ds[144]", id, TextForPlayer)){SendClientMessage(playerid, COLOR_VIOLET, "Пример использования пинга: {CCFF00}(@/#/№)ID сообщение"); return 0;}
		if(!IsPlayerConnected(id)){SendClientMessage(playerid, COLOR_VIOLET, ""NS" {FFFFFF}Игрок не в сети!");return 0;}
		if(gPlayerSpawned[id] == 0){SendClientMessage(playerid, COLOR_VIOLET, ""NS" {FFFFFF}Игрок не заспавнился!");return 0;}
...
ch_gotos:
		...
		new Float:xyz[3]; GetPlayerPos(playerid, xyz[0],xyz[1],xyz[2]);
	...
	if(strcmp(cmd, "/mmd", true) == 0)
	{
		...
		if(GetPVarInt(playerid, "MnMode") == 1)
		{
			...
			for(new i=0;i<sizeof(playspax);i++){
		    	...
		    	SpawnTextDraw[playerid][i] = CreatePlayer3DTextLabel(playerid,string,0x8F30E4AA,playspax[i], playspay[i], playspaz[i],20.0);
			}
		}
		else
		{
	...
	if((newkeys & 1024 && GetPVarInt(playerid, "MnMode") != 2) || (newkeys & 65536 && GetPVarInt(playerid, "MnMode") == 2))
	{
		...
		if(GetPlayerInterior(playerid) == 0)
		{
			for(new i; i < MAX_CREATED_HOUSE; i++)
			{
				...
				{
				    new str[88]; format(str,sizeof str, "Этот дом продаётся! Хотите купить его за %d$?", HouseInfo[i][hCost]);
...
fpub:Freeze1sec(playerid){TogglePlayerControllable(playerid,true);}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	switch(dialogid)
	{
		...
		case 9892:
	    {
			...
			new str[100]; format(str,sizeof str, "Вы продали свой дом за %d$",cost);
	...
	if(dialogid == 4905){
		...
		for(new i = strlen(inputtext)-1; i != -1; i--)
		{
		    ...
		    {
		        case '0'..'9': continue;
		        default: return SendClientMessage(playerid, COLOR_VIOLET, ""NS" {CCFF00}Только цифры!");
		    }
	    }
	   	for(new i=0;i<MAX_BUSSINES;i++){
			if(IsPlayerInRangeOfPoint(playerid, 1.0, BI[i][xX], BI[i][yY], BI[i][zZ])){
	...
	if(dialogid == 302)
	{
		if(response)
		{
			...
			for(new i; i<sizeof(playspax);i++){
			    ...
			    SpawnTextDraw[playerid][i] = CreatePlayer3DTextLabel(playerid,string,COLOR_VIOLET,playspax[i], playspay[i], playspaz[i],20.0);
			}
...
CMD:report(playerid, params[])
{
	...
	if(CheckFlood(playerid, "StopReport", 20) == 0){
		...
		foreach(Player,i)
		{
			if(IsPlayerConnected(i))
			{
				if(AI[i][aLevel] >= 1) {per55 = 1;}//поиск только НЕ скрытых админов
			}
		}
...
CMD:try(playerid, params[])
{
	...
	switch(random(2))
	{
		...
		default: format(str, sizeof(str), "%s %s | {FF0000}Неудачно", PlayerInfo[playerid][pName], text);
	}
	new Float:X, Float:Y, Float:Z;
	GetPlayerPos(playerid, X, Y, Z);
	foreach(Player, i)
...
#tryinclude "../../modules/commands.inc"
